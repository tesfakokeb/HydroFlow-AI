...
export const useApp = create<AppState>((set, get) => ({
  ...
  async bootstrap() {
    // Public demo mode: if there is no token yet, log in with the built-in
    // demo account so every visitor can open the dashboard immediately.
    if (!getAccessToken()) {
      try {
        const demo = await api.demoCredentials();
        if (demo.enabled && demo.email && demo.password) {
          const { tokens, user } = await api.login(demo.email, demo.password);
          setTokens(tokens);
          set({ user, authChecked: false });
          await get().bootstrap();
          return;
        }
      } catch {
        // Fall through to the unauthenticated shell if the demo account is disabled.
      }
    ...
    try {
      ...
      const projectId =
        localStorage.getItem("hydro.projectId") ?? projects[0]?.id ?? null;
      ...
      set({
        ...
        unitSystem:
          projects.find((p) => p.id === projectId)?.unitSystem ??
          get().unitSystem,
        authChecked: true,
      });
    } catch {
