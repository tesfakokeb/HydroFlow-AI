...
export function loadConfig(): AppConfig {
  ...
  if (nodeEnv === "production" && !jwtSecret) {
    throw new Error(
      "JWT_SECRET must be set in production. Refusing to start with a development secret.",
    );
  }
  ...
  return {
    ...
    auth: {
      jwtSecret:
        jwtSecret ?? "development-only-secret-do-not-use-in-production",
      accessTokenTtl: env("ACCESS_TOKEN_TTL", "30m") ?? "30m",
      refreshTokenTtl: env("REFRESH_TOKEN_TTL", "30d") ?? "30d",
      allowDemoLogin: envBool("ALLOW_DEMO_LOGIN", true),
    },
    copilot: {
